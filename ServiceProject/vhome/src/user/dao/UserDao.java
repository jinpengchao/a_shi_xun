...
public class UserDao {
	...
	public void charujubaoren(ParentUserInfo pu) {
		DBUtil util = DBUtil.getInstance();
		Connection conn = null;
		PreparedStatement psmt = null;
		try {
			conn = util.getConnection();
			String sql = "insert into tbl_parentuser_reported values(?,?,?,?,?,?,?)";
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, pu.getPhone());
			psmt.setString(2, pu.getNikeName());
			psmt.setString(3, pu.getPersonalWord());
			psmt.setString(4, pu.getHeaderImg());
			psmt.setString(5, "-");
			psmt.setInt(6, 0);
			psmt.setInt(7, 0);
			int rs = psmt.executeUpdate();
